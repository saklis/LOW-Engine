...
namespace LowEngine::Input {
	...
	inline bool operator==(const ModifierKey& modifier, const Action& action) { return action == modifier; }
	...
	struct ActionKey {
		...
		ActionKey(sf::Keyboard::Key key, const ModifierKey& modifiers) : ActionKey(
			key, modifiers.LShift, modifiers.LCtrl, modifiers.LAlt, modifiers.RShift, modifiers.RCtrl,
			modifiers.RAlt) {}
	};
	...
	inline bool operator==(const ModifierKey& modifier, const ActionKey& key) { return key == modifier; }
}
...
namespace LowEngine::Input {
	...
	public:
		...
		void AddEmptyAction(const std::string& actionName);
		...
		void AddAction(const std::string& actionName, sf::Keyboard::Key key, Keys... modifiers) {
			...
			_log->debug("Action '{}' added with keyboard button {} and modifiers {}", actionName, static_cast<int>(key),
			            FormatKeysModifiers(_actions[actionName].LShift, _actions[actionName].LCtrl,
			                                _actions[actionName].LAlt, _actions[actionName].RShift,
			                                _actions[actionName].RCtrl, _actions[actionName].RAlt));
		}
		...
		void AddAction(const std::string& actionName, sf::Mouse::Button mouseButton, Keys... modifiers) {
			...
			_log->debug("Action '{}' added with mouse button {} and modifiers {}", actionName,
		...
		/**
		 * @brief Remove Action by its name.
		...
		/**
		 * @brief Retrieve all defined Actions.
		 *
		 * This function returns a pointer to the internal map of Actions.
		...
		/**
		 * @brief Check if Action with provided name exists.
		 * @param actionName Name of the Action to check.
		 * @return true if Action with provided name exists, false otherwise.
		 */
		...
		/**
		 * @brief Serialize all Actions to JSON format.
		 *
		 * This function is used to serialize the current state of all defined Actions into a JSON object.
		 * @return nlohmann::ordered_json containing serialized Actions.
		 */
		nlohmann::ordered_json SerializeActionsToJSON();

		/**
		 * @brief Load Actions from JSON format.
		 *
		 * This function is used to load Actions from a JSON object.
		 * @param actionsJson nlohmann::basic_json<nlohmann::ordered_map> containing serialized Actions.
		 * @return true if loading was successful, false otherwise.
		 */
		bool LoadActionsFromJSON(const nlohmann::basic_json<nlohmann::ordered_map>& actionsJson);

		/**
		 * @brief Get name of the key.
		 * @param key Key to get name for.
		 * @return Name of the key as a string.
		 */
		std::string GetKeyName(sf::Keyboard::Key key);

		/**
		 * @brief Get name of the mouse button.
		 * @param button Mouse button to get name for.
		 * @return Name of the mouse button as a string.
		 */
		std::string GetMouseButtonName(sf::Mouse::Button button);
